...
const I18N = {
  en: {
    ...
    status_override_sub: 'These StringTables files have been overwritten. When the game is set to one of these languages, it uses that voice while reading text from your selected text language.',
    step_caption_title: 'Text Language',
    step_caption_sub: 'The text language you want to keep',
    step_voice_title: 'Voice Language Slot',
    step_voice_sub: 'The in-game language to use for voice; its text file will be replaced',
    ...
    modal_swap_body: (d, v) => `Copy <b>${d.name}</b> text into the <b>${v.name}</b> language slot.<br>Then set the in-game language to <b>${v.name}</b> to use <b>${v.name}</b> voice with <b>${d.name}</b> text.<br><span style="color:var(--text-mute);font-size:12px">Original ${v.name} StringTables file is backed up to ${v.code}.zip.bak</span>`,
  ...
  zh: {
    ...
    status_override_sub: '這些 StringTables 文字檔已被覆寫；遊戲設定為下列語言時會使用該語音，但讀取你選擇的文字語言。',
    step_caption_title: '文字語言',
    step_caption_sub: '你想保留顯示文字的語言',
    step_voice_title: '語音語言槽',
    step_voice_sub: '進遊戲要設定來聽語音的語言；它的文字檔會被替換',
  ...
  ja: {
    ...
    status_override_sub: 'これらの StringTables テキストファイルは上書きされています。ゲームをこれらの言語に設定すると、その音声を使いながら、選択したテキスト言語を読み込みます。',
    step_caption_title: 'テキスト言語',
    step_caption_sub: '表示テキストとして残したい言語',
    step_voice_title: '音声言語スロット',
    step_voice_sub: '音声用にゲーム内で設定する言語。このテキストファイルが置き換えられます',
  ...
  es: {
    ...
    status_override_sub: 'Estos archivos de texto StringTables fueron sobrescritos. Si configuras el juego en uno de estos idiomas, usará esa voz mientras lee el idioma de texto seleccionado.',
    step_caption_title: 'Idioma del texto',
    step_caption_sub: 'El idioma del texto que quieres conservar',
    step_voice_title: 'Espacio de idioma de voz',
    step_voice_sub: 'El idioma que debes seleccionar en el juego para la voz; su archivo de texto será reemplazado',
  ...
  ko: {
    ...
    status_override_sub: '이 StringTables 텍스트 파일들은 덮어쓰기되었습니다. 게임 언어를 해당 언어로 설정하면 그 음성을 사용하면서 선택한 텍스트 언어를 읽습니다.',
    step_caption_title: '텍스트 언어',
    step_caption_sub: '유지하고 싶은 표시 텍스트 언어',
    step_voice_title: '음성 언어 슬롯',
    step_voice_sub: '음성을 위해 게임 안에서 설정할 언어; 이 텍스트 파일이 교체됩니다',
