...
const I18N = {
  en: {
    ...
    modal_swap_body: (d, v) => `You will see <b>${d.name}</b> text and hear <b>${v.name}</b> voice.<br>After the swap, set the in-game language to <b>${v.name}</b>.<br><span style="color:var(--text-mute);font-size:12px">A backup is created automatically before changing anything.</span>`,
  ...
  zh: {
    ...
    modal_swap_body: (d, v) => `你將會看到 <b>${d.name}</b> 文字，並聽到 <b>${v.name}</b> 語音。<br>替換完成後，請進遊戲把語言設為 <b>${v.name}</b>。<br><span style="color:var(--text-mute);font-size:12px">修改前會自動建立備份。</span>`,
  ...
  ja: {
    ...
    modal_swap_body: (d, v) => `<b>${d.name}</b> の文字を表示し、<b>${v.name}</b> の音声を聞けるようにします。<br>完了後、ゲーム内言語を <b>${v.name}</b> に設定してください。<br><span style="color:var(--text-mute);font-size:12px">変更前にバックアップを自動作成します。</span>`,
  ...
  es: {
    ...
    modal_swap_body: (d, v) => `Verás texto en <b>${d.name}</b> y oirás voces en <b>${v.name}</b>.<br>Después del cambio, configura el idioma del juego en <b>${v.name}</b>.<br><span style="color:var(--text-mute);font-size:12px">Se crea una copia de seguridad automáticamente antes de cambiar nada.</span>`,
  ...
  ko: {
    ...
    modal_swap_body: (d, v) => `<b>${d.name}</b> 텍스트를 보고 <b>${v.name}</b> 음성을 듣도록 설정합니다.<br>완료 후 게임 언어를 <b>${v.name}</b>로 설정하세요.<br><span style="color:var(--text-mute);font-size:12px">변경 전 자동으로 백업을 만듭니다.</span>`,
